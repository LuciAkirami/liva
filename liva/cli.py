...
def main():
    ...
    speech_processor = SpeechProcessor(device=device, stt_model=args.stt_model)
    model = LLMInference(model_id=args.model_id, url=args.url, api_key=args.api_key)
    ...
    while True:
        ...
        for item in speech_processor.transcribe_audio(
            mic, generate_kwargs={"max_new_tokens": 128}
        ):
